...
- Fundraising: 펀드레이징, LP 이슈, GP 선정 관련
  (블라인드/프로젝트 펀드 결성·클로징, 위탁운용사 선정·숏리스트·출자사업,
   국민연금·우본·공제회·모태펀드·성장금융 등 LP 동향, 운용사 인가/설립, AUM·드라이파우더 집계)
...
> ⚠️ 규칙 1을 적용하기 전에 규칙 8을 먼저 확인하세요.
> 운용사가 **펀드를 결성하거나 출자자에게 선정되는** 기사는 기업명과 "투자"가 등장해도
> 규칙 1이 아니라 규칙 8(fundraising)입니다. 가장 흔한 오분류입니다.
...
**판별 질문: 이 기사의 주인공이 "돈을 굴리는 쪽(운용사·LP)"인가, "사고팔리는 회사"인가?**
운용사가 돈을 모으거나 / 배정받거나 / 선정되는 이야기면 무조건 fundraising입니다.
기사에 특정 기업명과 "투자"라는 단어가 있다고 해서 규칙 1로 보내지 마세요.

대상 (아래 표현이 제목·본문의 핵심이면 fundraising):
- **GP 선정·출자사업**: 위탁운용사 선정/선발, GP 선정/탈락, 숏리스트, 1차·2차 관문,
  출자사업, 정시 출자, 수시 출자, 위탁운용사 공고, 제안서 접수, 운용사 모집
- **출자자(LP)**: 국민연금, 우정사업본부(우본), 공무원연금, 사학연금, 교직원공제회,
  군인공제회, 과학기술인공제회, 노란우산, 산업은행, 모태펀드(한국벤처투자),
  한국성장금융, 농금원, 신보/기보, 지자체 펀드 — 이들의 출자·배분·심사·정책
- **펀드 결성**: 블라인드펀드, 프로젝트펀드, 병행펀드, 재간접펀드, 코인베스트먼트 펀드,
  결성총회, 1차 클로징, 최종 클로징, 앵커 LP 확보, GP 커밋, 펀드 규모 증액
- **운용사 라이선스·조직**: 신기술사업금융업 등록, 전문사모 운용사 인가, 운용사 설립,
  하우스 출범, 대표·심사역 영입(운용사 자체 이야기일 때)
- **업계 집계**: AUM 순위, 드라이파우더, 펀드 소진율, 결성액/회수액 집계, 리그테이블
  (단, 개별 딜 회수 성과가 핵심이면 규칙 2의 exit)

예시:
  ✅ "○○PE 1조원 블라인드펀드 결성" → fundraising
  ✅ "국민연금 PE 출자 확대" → fundraising
  ✅ "신규 GP 운용사 설립 러시" → fundraising
  ✅ "우본, 사상 첫 대형주 운용사 선발…숏리스트 면면은" → fundraising (위탁운용사 선정)
  ✅ "이앤인베, 방산·농모태 GP…밸류업 전략 통했다" → fundraising (GP 선정)
  ✅ "크레센도, 해외 병행펀드 클로징 임박…테마섹·GIC 확보" → fundraising (펀드 결성)
  ✅ "1600억 3호 펀드 결성…AI·로봇·방산 중심 투자" → fundraising
     (투자 대상 산업이 제목에 있어도 핵심은 '펀드 결성')
  ✅ "VC의 PEF AUM 17조대…아주IB투자, 1조 클럽 합류" → fundraising (업계 집계)
  ✅ "모험자본 곳간 반년 만에 1조 감소" → fundraising (LP/드라이파우더 동향)

  ❌ "○○PE, △△기업 경영권 인수" → acquisition_investment (사고파는 이야기)
  ❌ "○○PE, △△기업 지분 매각해 3배 회수" → exit (개별 딜 회수)
...
2. **"Investors - PEF/VC" 섹션 기사**를 무조건 한 곳에 넣지 마세요:
   ...
   - PE 업계 전반 동향 → Deal > 기타
3. **GP/위탁운용사 선정 기사를 Deal로 보내지 마세요.** "○○운용사가 △△ 출자사업
   위탁운용사로 선정" 같은 기사는 운용사가 자금을 배정받는 이야기이므로 fundraising입니다.
   "선정", "선발", "숏리스트", "출자사업", "위탁운용사"가 제목에 있으면 fundraising을 먼저 검토하세요.
4. **"Investors - IB" 섹션 기사**도 마찬가지:
